...
const DesignCanvas: React.FC<DesignCanvasProps> = ({
	...
	const defaultStorageDevices = defaultDevices.filter(
		(device) => device.category === DEVICE_CATEGORIES.STORAGE
	);
	const defaultTransformerDevices = defaultDevices.filter(
		(device) => device.category === DEVICE_CATEGORIES.TRANSFORMER
	);
	...
	const redrawCanvas = useCallback(() => {
		...
		if (canvas && context) {
			...
			// Draw buffer
			if (showBuffer) {
				const { bufferXPos, bufferYPos, bufferWidth, bufferLength } =
					getBufferAreaDimensions(systemLayout);
				drawBufferArea(
					context,
					bufferXPos,
					bufferYPos,
			...
			systemLayout.forEach((row, rowIndex) => {
				...
				row.forEach((device, deviceIndex) => {
					...
					if (deviceIndex < row.length - 1) {
						...
						if (
							device.category === DEVICE_CATEGORIES.TRANSFORMER &&
							nextDevice.category === DEVICE_CATEGORIES.STORAGE
						) {
							xPos += device.width + SPACING.WALKWAY_WIDTH;
						} else if (
							device.category === DEVICE_CATEGORIES.TRANSFORMER &&
							nextDevice.category ===
								DEVICE_CATEGORIES.TRANSFORMER
						) {
	...
	const handleCenterDevices = useCallback(() => {
		if (systemLayout.length === 0) return;
		const dpr = window.devicePixelRatio || 1;
		const deviceMinX = Math.min(
			...systemLayout.flat().map((d) => d.x || 0)
		);
		const deviceMinY = Math.min(
			...systemLayout.flat().map((d) => d.y || 0)
		);
		const deviceMaxX = Math.max(
			...systemLayout.flat().map((d) => (d.x || 0) + d.width)
		);
		const deviceMaxY = Math.max(
			...systemLayout.flat().map((d) => (d.y || 0) + d.length)
		);
		...
		const centeredOffsetX =
			(canvasWidth - layoutWidth * newScale) / 2 - deviceMinX * newScale;
		const centeredOffsetY =
			(canvasHeight - layoutHeight * newScale) / 2 -
			deviceMinY * newScale;

		setScale(newScale / dpr);
		setOffsetX(centeredOffsetX / dpr);
		setOffsetY(centeredOffsetY / dpr);
	}, [systemLayout]);
	...
	const getTooltipContent = (device: Device): JSX.Element => {
		return (
			<div className="flex flex-col border rounded bg-white p-2">
				...
				<div>{numberToString(device.capacity_kwh)} kWh</div>
				<div className="text-sm">
					{numberToMoneyString(device.cost)}
				</div>
				<img
					className="mt-2 max-w-40 rounded"
					src={`/assets/products/${device.img}`}
					alt=""
				/>
			</div>
		);
	...
	const renderTooltip = () => {
		if (!tooltip || !canvasRef?.current) return null;
		const dpr = window.devicePixelRatio || 1;
		...
		return (
			<div
				style={{
					position: 'absolute',
					left: tooltip.x - canvasRef.current.width / 3 / dpr,
					top: tooltip.y - canvasRef.current.height / 3 / dpr,
					zIndex: 10,
					pointerEvents: 'none',
	...
	return (
		<div className="flex flex-col grow rounded-xl border-2 min-w-1/2 w-3/4  mt-4 mr-4 ml-4 relative">
			...
			<div className="absolute right-4 top-4 flex flex-col gap-4">
						...
						<div className="pl-4 font-bold">
							{
								systemLayout
									...
									.filter(
										(d) =>
											d.name ===
											defaultTransformerDevices[0].name
									).length
							}
						</div>
					</div>
				...
				<Button
					variant="outlined"
					aria-label="center devices on canvas"
					onClick={handleCenterDevices}
				>
				...
				<Button
					variant="outlined"
					aria-label="show buffer area on devices"
